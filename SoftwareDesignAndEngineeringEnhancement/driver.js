...
// adding sample animals to table
let monkey1 = new Monkey("Max", "Marmoset", "Male", "2", "20", "12", "12", "12", "2025-02-08", "United States", "Intake", false, "United States");
let monkey2 = new Monkey("Lucky", "Guenon", "Male", "1", "23", "12", "12", "12", "2025-01-08", "United States", "Intake", false, "United States");
let monkey3 = new Monkey("Missy", "Capuchin", "Female", "3", "21", "12", "12", "12", "2024-02-08", "United States", "In-Service", true, "United States");

// created function to append table data to newley created table row
function addMonkeySamples(monkey) {
    ...
    newMonkeyRow.innerHTML = `
                            <td>${monkey.name}</td>
                            <td>${monkey.species}</td>
                            <td>${monkey.gender}</td>
                            <td>${monkey.age}</td>
                            <td>${monkey.weight}</td>
                            <td>${monkey.trainingStatus}</td>
                            <td>${monkey.reserved}</td>
                            <td>${monkey.inServiceLocation}</td>
                            `;

    // add the inner html to our new table row element
    monkeyTable.appendChild(newMonkeyRow);
}

//Adding the examples into our table through html
addMonkeySamples(monkey1);
addMonkeySamples(monkey2);
addMonkeySamples(monkey3);
...
function displayMonkeyIntakeform() {
    ...
    monkeyForm.addEventListener("submit", (e) => {
        ...
        // grabs the div of the monkey data table
        const monkeyTable =  document.getElementById("monkey-table");
        ...
        // adds the new table row to the table along with its contents
        monkeyTable.appendChild(newMonkeyRow);
